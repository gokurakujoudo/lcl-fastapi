...
| LCL configuration | [Configuration](configuration.md) | Formal upstream loading, config-relative paths, framework-owned worker PID, supported IPv4 binds, and LCL environment expressions for non-overlapping machine ID ranges. |
...
| CLI and shutdown | [CLI](cli.md), [runtime](runtime.md) | All six operations use lclang syntax, reject unsupported overrides, keep diagnostics off structured stdout, reject wrong control tokens, and complete graceful whole-service shutdown. |
